...
public class EMFImageLoader {
    ...
    */
   public static BufferedImage readImage( BitmapInfoHeader bmi, int width, int height, EMFInputStream emf,
         int len, BlendFunction blendFunction ) throws IOException
   {
      ...
      {
         return handle1Bit( emf, width, height, len );
      }
      else if( ( bmi.getBitCount() == 8 ) && ( bmi.getCompression() == EMFConstants.BI_RGB ) )
      {
         return handle8Bit_RGB( emf, width, height, len, bmi );
      }
      ...
      // NULL.
      else if( ( bmi.getBitCount() == 16 ) && ( bmi.getCompression() == EMFConstants.BI_RGB ) )
      {
         return handle16Bit_RGB( emf, width, height, len );
      }
      else if( ( bmi.getBitCount() == 24 ) && ( bmi.getCompression() == EMFConstants.BI_RGB ) )
      {
         return handle24Bit_RGB( emf, width, height, len, bmi );
      }
      ...
      else if( ( bmi.getBitCount() == 32 ) && ( bmi.getCompression() == EMFConstants.BI_RGB ) )
      {
         return handle32Bit_RGB( emf, width, height, len, blendFunction );
      }
      ...
      else if( ( bmi.getBitCount() == 32 ) && ( bmi.getCompression() == EMFConstants.BI_BITFIELDS ) )
      {
   ...
   protected static BufferedImage handle1Bit( EMFInputStream emf, int width, int height, int len )
         throws IOException
   {
   ...
   protected static BufferedImage handle8Bit_RGB( EMFInputStream emf, int width, int height, int len,
         BitmapInfoHeader bmi ) throws IOException
   {
      ...
      // data a indexes to a certain color in the color table.
   ...
   protected static BufferedImage handle16Bit_RGB( EMFInputStream emf, int width, int height, int len )
         throws IOException
   {
      ...
      // maybe that has something to do with the HALFTONE rendering setting.
      ...
      // found no sample and color model to make this work
      ...
      for( int y = height - 1; y > -1; y--, off = off + width )
      {
         for( int x = 0; x < width; x++ )
         {
            ...
            // and set the resulting color values
            result.setRGB( x, y,
                  new Color(
                        // 0xF800 = 2 * 0x7C00
   ...
   protected static BufferedImage handle24Bit_RGB( EMFInputStream emf, int width, int height, int len,
         BitmapInfoHeader bmi ) throws IOException
   {
   ...
   protected static BufferedImage handle32Bit_RGB( EMFInputStream emf, int width, int height, int len,
      ...
      if( blendFunction.getAlphaFormat() != EMFConstants.AC_SRC_ALPHA )
      {
         ...
         for( int y = height - 1; y > -1 && off < data.length; y-- )
         {
            for( int x = 0; x < width && off < data.length; x++ )
            {
               ...
               result.setRGB( x, y,
                     new Color( ( pixel & 0xFF0000 ) >> 16, ( pixel & 0xFF00 ) >> 8, ( pixel & 0xFF ),
                           // TODO not tested
                           sourceConstantAlpha ).getRGB() );
            }
         }
      }
         ...
         if( sourceConstantAlpha == 0xFF )
         {
            ...
            for( int y = height - 1; y > -1 && off < data.length; y-- )
            {
               for( int x = 0; x < width && off < data.length; x++ )
               {
                  ...
                  result.setRGB( x, y, new Color( ( pixel & 0xFF0000 ) >> 16, ( pixel & 0xFF00 ) >> 8,
                        ( pixel & 0xFF ), alpha ).getRGB() );
               }
            }
         }
