 ...
 * @version $Id: freehep-graphics2d/src/main/java/org/freehep/graphics2d/GenericTagHandler.java 7aee336a8992 2005/11/25 23:19:05 duns $
 */
...
//    }
     ...
     * vertical <v>, overline <over> and typewriter <tt>
     */
    ...
    protected String openTag(String tag) {
        ...
        if (tag.equalsIgnoreCase("b")) {
            ...
            y -= font.getLineMetrics(mM, fontContext).getAscent()
                    * scriptShiftRatio;
            fontStack.push(font);
            graphics.setFont(font.deriveFont(font.getSize2D()
                    * scriptSizeFactor));
    ...
    protected String closeTag(String tag) {
        if (tag.equalsIgnoreCase("b")) {
            ...
            graphics.setFont((Font) fontStack.pop());
            y += graphics.getFont().getLineMetrics(mM,
                    graphics.getFontRenderContext()).getAscent()
                    * scriptShiftRatio;
        } else if (tag.equalsIgnoreCase("sub")) {
            graphics.setFont((Font) fontStack.pop());
            y -= graphics.getFont().getLineMetrics(mM,
    ...
    protected String text(String text) {
        Font font = graphics.getFont();
        FontRenderContext fontContext = graphics.getFontRenderContext();
        LineMetrics metrics = graphics.getFont().getLineMetrics(text,
                fontContext);
        float width = (float) font.getStringBounds(text, fontContext)
                .getWidth();
